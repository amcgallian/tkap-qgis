...
class EmlidToSuPlugin(object):
    ...
    def actions(self, parent):
        ...
        self.action = QAction(icon, "Emlid to SU…", parent)
        self.action.setObjectName("EmlidToSuAction")
    ...
    def run(self):
        ...
        from .emlid_to_su_dialog import EmlidToSuDialog

        # Reuse one dialog so field/layer selections persist during the session.
        if self.dialog is None:
            self.dialog = EmlidToSuDialog(self.iface, self.iface.mainWindow())
        self.dialog.show()
        self.dialog.raise_()
